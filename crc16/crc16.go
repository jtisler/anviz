...
func Crc16(bs []byte) string {
	l := len(bs)
	crc := uint16(0xffff)
	for i := 0; i < l; i++ {
		crc = crc ^ uint16(bs[i])
		crc = ((crc >> 8)) ^ crc16tab[((crc & 255))]
	}

	//convert uint16 to hexstring
	crc16 := fmt.Sprintf("%04X", crc)

	//swap bytes
	crc16 = fmt.Sprintf("%c%c%c%c", crc16[2], crc16[3], crc16[0], crc16[1])
